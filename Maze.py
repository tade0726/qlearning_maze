...
class Maze(object):
    ...
    def __init__(
        self, from_file=None, maze_size=None, trap_number=5, unstable_action=False
    ):
        """
    ...
    def __generate_maze(self, height=21, width=27, complexity=0.25, density=0.25):
        """
            ...
            for j in range(0, c - 2, 2):
                maze_data[i // 2, j // 2] = sum(
                    ...
                    for i, block in enumerate(
                        np.ravel(Z[i : i + 3, j : j + 3], order="F")[[3, 7, 5, 1]]
                    )
                )
    ...
    def __validate_maze(self):
        ...
        for r in range(height):
            for c in range(width - 1):
                if (self.maze_data[r, c] & 2 != 0) != (
                    self.maze_data[r, c + 1] & 8 != 0
                ):
        ...
        if wall_errors:
            for cell, wall_type in wall_errors:
                if wall_type == "v":
                    ...
                    print(
                        "Inconsistent horizontal wall betweeen {} and {}".format(
                            cell, cell2
                        )
                    )
    ...
    def __draw_raw_maze_img(self):
        ...
        maze = np.vstack(
            (np.hstack((grid_images[i] for i in row)) for row in self.maze_data)
        )
    ...
    def move_robot(self, direction):
        ...
        if self.is_permissible(self.robot["loc"], direction):
            self.robot["loc"] = tuple(
                (i + di for i, di in zip(self.robot["loc"], self.move_map[direction]))
            )
