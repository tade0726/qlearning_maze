...
class Runner(object):
    ...
    robot_img = {
        d: imageio.imread(join("images/", "robot-" + d + ".jpg")) for d in valid_actions
    }
    logo_img = imageio.imread("images/logo.jpg")
    arrow_img = {
        d: imageio.imread(join("images/", "arrow-" + d + ".jpg")) for d in valid_actions
    }
    ...
    def run_training(
        self, training_epoch, training_per_epoch=150, display_direction=False
    ):

        self.train_robot_record = {}
        self.train_robot_statics = {"success": [], "reward": [], "times": []}
        ...
        def train_logger_before_act(e, i):
            ...
            self.train_robot_record[(e, i)]["qtable"] = self.robot.Qtable[
                self.robot.sense_state()
            ].copy()
    ...
    def draw_header(self, base_image):
        ...
        logo_image = np.vstack(
            (Image.new("RGB", (200, 50), color=(255, 255, 255)), self.logo_img)
        )
        logo_image = np.vstack(
            (logo_image, Image.new("RGB", (200, 50), color=(255, 255, 255)))
        )
        header_shape = (
            base_image.shape[1] - logo_size,
            logo_size + 100,
        )  # width, height
        header_img = np.hstack(
            (logo_image, Image.new("RGB", header_shape, color=(255, 255, 255)))
        )
        return header_img
    ...
    def draw_robot(self, base_image, parameters):
        ...
        r, c = robot["loc"]
        img[
            r * grid_size : (r + 1) * grid_size, c * grid_size : (c + 1) * grid_size, :
        ] += self.robot_img[robot["dir"]]
        if self.display_direction:
    ...
    def generate_movie(self, filename):
        ...
        def ensemble_image(train_id):
            new_header = self.write_on_header(
                header_img, self.train_robot_record[train_id]
            )
            current_maze = self.draw_robot(
                base_image, self.train_robot_record[train_id]
            )
        ...
        writer = cv2.VideoWriter(
            filename, cv2.VideoWriter_fourcc(*"XVID"), 10.0, (width // 2, height // 2)
        )

        with tqdm.tqdm(
            sorted(self.train_robot_record.keys()), desc="Generate Movies"
        ) as pbar:
            for key in pbar:
